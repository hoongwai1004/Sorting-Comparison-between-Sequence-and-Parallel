...
void docuda(int *dnum, int threads){
	int i;
	for(i = 0; i < LINE; i++){
		even<<<LINE, threads>>>(dnum, LINE);
		odd<<<LINE, threads>>>(dnum, LINE);
	}
}
...
void cuda(int num[LINE], int num1[LINE]){
	...
	threads = 1;
	block = LINE;
	printf("Time execution for parallel bubble sort using CUDA using 100k block based on threads per block\n");
	printf("====================================================================================================\n");
